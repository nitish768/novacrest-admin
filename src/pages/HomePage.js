...
const HomePage = () => {
  ...
  return (
    <>
      ...
      <div className="home-cta">
        <div className="home-cta-inner">
          ...
          <div className="home-cta-buttons">
            <Link className="btn hero-btn-solid" to="/order">Place Your Order</Link>
            <Link className="btn hero-btn-ghost" to="/products">Browse Products</Link>
          </div>
          <div className="home-cta-trust">
            {ctaTrust.map((item, i) => (
              <span key={i} className="cta-trust-item">{item}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      {showModal && (
        <VisitingCardModal cardImage={cardImage} onClose={() => setShowModal(false)} />
      )}
    </>
  );
};
